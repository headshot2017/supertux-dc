...
//----------------------------------------------------
{
	...
	for (;;)
	{
		...
		MODCOMMAND *p = Patterns[nPattern] + nRow * m_nChannels;
            ...
            // Recommendation by mrneo240:
            // "If you cast it as a dword then deref, it isn't required to check it's aligned for you"
            // "You could do a 4byte memcpy to be very safe"
            DWORD temp;
            memcpy(&temp, p, 4);

		for (UINT nChn=0; nChn<m_nChannels; p++,pChn++, nChn++) if (temp)
		{
